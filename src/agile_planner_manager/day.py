...
from task import Task, Task.SubTask
...
class Day:
    ...
    def add_subtask(self, task):
        if self.date == task.due_date:
            overflow = task.get_subtotal_remaining() > get_spare_hours()
            hours = task.get_subtotal_remaining()
            subtask = task.add_sub_task(hourse, overflow)
    ...
    def num_subtask(self):
        return subtask_manager.size()

    def has_spare_hours(self):
        return get_spare_hours() > 0
